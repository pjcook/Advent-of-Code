...
public class PriorityQueue<T: Hashable> {
    public init() {}
    private class QueueItem: Comparable {
        let item: T
        let priority: Int
        
        init(item: T, priority: Int) {
            self.item = item
...
//    public var queue = [T:Int]()
    private var queue = BinaryHeap<QueueItem>(comparator: <)
...
//        queue[item] = priority
        queue.insert(QueueItem(item: item, priority: priority))
    }
